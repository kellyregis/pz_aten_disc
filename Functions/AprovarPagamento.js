    ...
    for (const entrega of yy22) {
        ...
        if (autoentrega == true) {
            ...
            try {
                ...
                .setDescription(`O usuário <@!${yy.user.id}> realizou uma compra no servidor`)
                .addFields(
                ...
                try {
                    setTimeout(async () => {
                        ...
                        if (dd !== null) {
                            ...
                            await member.send({ components: [row6aa], content: `Olá, <@!${member.id}>, deu tudo certo como deveria? Não se esqueça de nos dar seu feedback, caso ainda não tenha feito.` })
                        }
                    }, 60000);
                } catch (error) {
